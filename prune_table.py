...
def table_phase0():
    print('start')
    trans_ep = [[-1 for _ in range(14)] for _ in range(495)]
    ...
    with open('prune_table/prune_phase0_co_ep.txt', mode='w') as f:
        for arr in table:
            for elem in arr:
                f.write(str(elem) + '\n')
    ...
    while que:
        ...
        for twist_idx, twist in enumerate(candidate[0]):
            ...
            if table[n_idx1][n_idx2] > cost:
                table[n_idx1][n_idx2] = cost
                que.append([n_idx1, n_idx2, cost])
    with open('prune_table/prune_phase0_eo_ep.txt', mode='w') as f:
        for arr in table:
            for elem in arr:
                f.write(str(elem) + '\n')
    print('phase0 done')
